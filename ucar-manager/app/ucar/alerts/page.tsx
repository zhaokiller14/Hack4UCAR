...
export default async function UcarAlerts() {
	...
	const acknowledgedCount = alerts.filter((item) => item.is_acknowledged).length;
	...
	const topSeverities = Object.entries(severityCounts).sort((a, b) => b[1] - a[1]);
		...
		<main className="mx-auto max-w-7xl space-y-8 p-8">
			<div>
				...
				<p className="mt-0.5 text-sm text-slate-500">
					Vue statistique des alertes réseau détectées sur les KPIs institutionnels.
				</p>
			</div>
			...
			<div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
				<SectionCard title="Répartition par sévérité" description="Volume d'alertes par niveau de criticité">
					<div className="space-y-3">
						{topSeverities.length === 0 && <p className="text-sm text-slate-500">Aucune alerte disponible.</p>}
						{topSeverities.map(([severity, count]) => (
							<div key={severity} className="space-y-1">
								<div className="flex items-center justify-between gap-3">
									<SeverityBadge severity={severity} />
									<span className="text-xs text-slate-500">{count} alertes</span>
								</div>
				...
				<SectionCard title="Répartition par domaine KPI" description="Domaines les plus exposés aux anomalies">
					<div className="space-y-3">
			...
			<SectionCard title="Dernières alertes" description="Flux temps réel des alertes UCAR">
							...
							{alerts.map((item) => {
								...
								return (
									<tr key={item.id} className="border-t border-[#003850]/10 align-top">
										...
										<td className="px-2 py-2 text-slate-600">{item.kpi_domain}</td>
